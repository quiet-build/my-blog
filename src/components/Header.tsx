import { forwardRef, useCallback, useEffect, useRef, useState, type MouseEvent as ReactMouseEvent } from 'react'
        ...
        {links.map(
          link =>
            link.show && (
              ...
              >
                {link.external
                  ? <a href={link.to} target="_blank" rel="noreferrer">{link.name}</a>
...
export default function Header ({ navBarTitle, fullWidth }: HeaderProps) {
  ...
  const resolveFavicon = (fallback?: boolean) => !fallback && dark ? '/favicon.dark.png' : '/favicon.png'
  ...
  function handleClickHeader (ev: ReactMouseEvent<HTMLDivElement>) {
    if (ev.target !== navRef.current && ev.target !== titleRef.current) return
